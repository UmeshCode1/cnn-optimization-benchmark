...
if ((Test-Path "$SCRIPT_DIR\backend\main.py") -and (Test-Path "$SCRIPT_DIR\frontend")) {
    ...
    Write-Host "[SETUP] Running in local repository folder: $INSTALL_DIR" -ForegroundColor Green
} else {
    $INSTALL_DIR = "$HOME\cnn-benchmark-local"
    $IS_LOCAL_SOURCE = $false
    Write-Host "[SETUP] Target installation directory: $INSTALL_DIR" -ForegroundColor Cyan
}
...
if (-not $PYTHON_CMD) {
    ...
    try {
        ...
        Write-Host "  Please install Python 3.10+ from https://www.python.org/downloads/ (check 'Add python.exe to PATH') and re-run." -ForegroundColor Yellow
        pause
        Exit 1
    }
}

# ── 3. Clone or Validate Repository Files ─────────────────────────────────────
...
Write-Host "  * Installing core backend web framework & scientific dependencies..." -ForegroundColor Gray
...
try {
    if ($HAS_NVIDIA) {
        ...
        Write-Host "    -> Installing CPU-optimized PyTorch (fast lightweight wheel)..." -ForegroundColor Gray
        & $VENV_PYTHON -m pip install torch torchvision --index-url https://download.pytorch.org/whl/cpu -q
    }
} catch {
    Write-Host "    [!] PyTorch installation encountered a warning. Simulation/Demo mode remains fully operational." -ForegroundColor Yellow
}
...
Write-Host "  Interactive Doc: http://localhost:8000/docs" -ForegroundColor Gray
