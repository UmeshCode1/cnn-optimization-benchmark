...
Where $\hat{T}_d$ is the target value in the $d$-th dimension.

The social attraction and repulsion function $s(r)$ is defined as:
...
Where the Math Optimizer Probability is defined as:

$$\text{MOP}(t) = 1 - \left(\frac{t}{T}\right)^{1/\alpha}$$
